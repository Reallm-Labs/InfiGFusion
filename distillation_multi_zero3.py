...
from distiller_multi import Distiller
from data_utils.distill_datasets_multi import DistillDataset
from utils import (
    initialize,
    get_optimizer, 
...
def prepare_dataset(args, distiller):
    ...
    if args.do_train:
        data["train"] = DistillDataset(
            args, args.train_json, distiller.student_tokenizer,
            distiller.teacher_tokenizers_dict
        )
...
def finetune(
    ...
    log_rank(f"[Rank {dist.get_rank()}] Entering training loop.")
    print(f"[Rank {dist.get_rank()}] ...", flush=True)
    ...
    # currently, for debugging, set shuffle=False
    # make sure we need all the data logits extracted, else
    # shuffle will make it errors
    ...
    for epoch in range(args.num_epochs):
        ...
        while True:
#            if epoch == 4: # the final epoch
#                log_rank("DO not need train the 5-th epoch, save time...")
...
#                break
            ...
            for i in range(args.gradient_accumulation_steps):
                try:
                    ...
                    log_rank("Epoch {} done training in {:0>2}:{:0>2}:{:0>2}".format(
                        epoch+1,
                        int(elapsed_epoch_time // 3600), 
                        int(elapsed_epoch_time % 3600 // 60), 
                        int(elapsed_epoch_time % 60)
                    ))
                    end_epoch = True
                    break
            ...
            dist.all_reduce(global_token_num, dist.ReduceOp.SUM, group=dp_group) # 用于在多个进程或 GPU 间汇总数据，通常应用于计算累积的全局统计信息
            ...
            for batch in global_batch:
                ...
                model.backward(loss)
                model.step()
                if torch.cuda.is_available() and epoch_step % 10 == 0:
        ...
        if args.save_dir and (epoch + 1) % args.save_interval == 0:
            ...
            # 使用 GatheredParameters 收集分布在各个 GPU 上的参数
            with deepspeed.zero.GatheredParameters(model.module.student_model.parameters()):
                if dist.get_rank() == 0:
            ...
            if hasattr(model.module, "projectors"):
                ...
                with deepspeed.zero.GatheredParameters(model.module.projectors.parameters()):
                    if dist.get_rank() == 0:
                        torch.save(
            ...
            if hasattr(model.module, "stu_lm_heads"):
                log_rank("Saving stu_lm_heads...")
                with deepspeed.zero.GatheredParameters(model.module.stu_lm_heads.parameters()):
                    if dist.get_rank() == 0:
                        torch.save(
                            model.module.stu_lm_heads.state_dict(), 
                            os.path.join(save_dir_path, "stu_lm_heads.pt")
                        )
...
def main():
    ...
    import socket
    if dist.is_initialized():
        print(f"[Rank {dist.get_rank()}] Initialized on host {socket.gethostname()}")
    else:
        print(f"[Single Process] Running on {socket.gethostname()}")
